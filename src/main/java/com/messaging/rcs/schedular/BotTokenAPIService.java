...
public class BotTokenAPIService {
	...
	public ResponseEntity<String> sendTemplateToOperator(String botId, String viAuthorization, Template templateJson,
			MultipartFile[] files) throws IOException {
		System.out.println("URL used for authentication is ::" + operatorTemplateApi.replace("{botId}", botId));
		...
		if (templateJson.getTemplateMsgType().equalsIgnoreCase("rich_card")
			...
			for (MultipartFile file : files) {
				System.out.println("Adding File In Bytes Inside multimedia_files Param ::" + file.getName());
		...
		return restTemplate.exchange(operatorTemplateApi.replace("{botId}", botId), HttpMethod.POST, requestEntity,
				String.class);
	}
	...
	public ResponseEntity<String> findTemplateStatusFromOperator(String viAuthorization, String templateName,
			String botId) {
		System.out.println("URL used for authentication is ::"
				+ approvedTempalteStatusApi.replace("{botId}", botId).replace("{name}", templateName));
		System.out.println("BOT ID:: " + botId);
		System.out.println("AUTHORIZATION :: " + viAuthorization);
		RestTemplate restTemplate = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		...
		return restTemplate.exchange(
				approvedTempalteStatusApi.replace("{botId}", botId).replace("{name}", templateName), HttpMethod.GET,
				entity, String.class);
	}

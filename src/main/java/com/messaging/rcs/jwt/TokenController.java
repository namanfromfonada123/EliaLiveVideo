...
public class TokenController {
	...
	public ResponseEntity<?> generateToken(@RequestBody AuthenticationRequest loginUser, HttpServletRequest request)
		...
		ApiResponse<AuthToken> response = new ApiResponse<>(200, "admin", user.get().getUserType(),
...
/*	@RequestMapping(value = "/token", method = POST)
	public ResponseEntity<?> createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest)
		...
		String jwt = null;
		if (userDetails.isMultipleLoginAllowed()) {
			jwt = tokenService.getToken(userDetails.getUsername());

			if (jwt == null || !StringUtils.hasLength(jwt)) {
				// jwt = jwtTokenUtil.generateToken(userDetails);
				tokenService.saveToken(userDetails.getUsername(), jwt);
			}
			try {
				jwtTokenUtil.validateToken(jwt, userDetails);
				LOGGER.info("Valid Token for " + userDetails.getUsername());

			} catch (Exception e) {
				LOGGER.info("Expired Token for " + userDetails.getUsername() + " regenrating");
